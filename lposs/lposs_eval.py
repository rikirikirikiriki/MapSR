...
from mmcv.image import imresize as resize
...
def get_lposs_laplacian(feats, locations, height_width, sigma=0.0, pix_dist_pow=2, k=100, gamma=1.0, alpha=0.95, patch_size=16):
    ...
    # Handle invalid indices (e.g. -1) returned by faiss
    valid_mask = (ks >= 0) & (ks < N)
    
    # Create safe indices for gather (replace -1 with 0)
    ks_safe = ks.clone()
    ks_safe[~valid_mask] = 0
    
    sims[sims < 0] = 0
    sims = sims ** gamma
    
    # Use safe indices for gather
    geometry_affinity = geometry_affinity.gather(1, ks_safe)
    
    # Mask out invalid connections in geometry_affinity and sims
    geometry_affinity[~valid_mask] = 0
    sims[~valid_mask] = 0
    
    geometry_affinity = geometry_affinity.flatten()
    sims = sims.flatten()
    sims = sims * geometry_affinity
    ks = ks.flatten()
    rows = torch.arange(N, device=feats.device).repeat_interleave(k)
    
    # Filter out invalid indices for sparse matrix construction
    valid_mask_flat = valid_mask.flatten()
    sims = sims[valid_mask_flat]
    rows = rows[valid_mask_flat]
    ks = ks[valid_mask_flat]
    
    # 将 CuPy 数组转换为 numpy，然后再创建 scipy.sparse.csr_matrix，最后转回 CuPy
    # 这是为了绕过某些 CuPy 版本的稀疏矩阵构建问题
    # 或者直接使用 CPU 构建，再转 GPU
    
    # 尝试 1: 确保所有输入都是 cupy array
    # W = csr_matrix(
    ...
    try:
        ...
        print(f"Error creating csr_matrix: {e}")
        # fallback: 使用 coo_matrix 然后转换
        W = coo_matrix(
            (cp.asarray(sims), (cp.asarray(rows), cp.asarray(ks))),
            shape=(N, N),
        ).tocsr()
