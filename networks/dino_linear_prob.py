...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, input_hidden_size, num_classes=17, chip_size=224):
        ...
        # Resolve local paths relative to the current script directory
        current_dir = Path(__file__).resolve().parent      # .../MapSR/networks/
        project_root = current_dir.parent                  # .../MapSR/
        ...
        # Load AnyUP upsampler from local directory
        self.upsampler = torch.hub.load(str(local_anyup_path), "anyup", verbose=False, source='local').eval()
