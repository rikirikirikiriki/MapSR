...
class DecoderCup(nn.Module):
    def __init__(self, config):
        ...
        #调控anyup的三个开关
        self.use_anyup=False#注意test的时候要改成false
        self.anyup_last_only=True #True就是仅最后一层用
        self.anyup_q_chunk_size=2 #显存不够时候减小q_chunk_size
        ...
        if self.use_anyup:
            try:
                ...
                self.anyup = torch.hub.load(
                    "wimmerth/anyup", "anyup_multi_backbone"
                )
            except Exception as e:
                logger.warning(
                    "AnyUp load failed, fallback to bilinear upsampling: %s", e
                )
                self.use_anyup = False
                self.anyup = None

    #def forward(self, hidden_states, features=None):
    def forward(self, hidden_states, features=None, input_image=None):#适配anyup
        ...
        #high-resolution image for AnyUp，取前 3个通道作为伪RGB
        hr_image = None
        if input_image is not None:
            if input_image.size(1) >= 3:
                hr_image = input_image[:, :3]
            else:
                hr_image = input_image

        #X is the output of ViT branch
        x = torch.cat([x, features[1]], dim=1)
        x = self.conv1(x)
        if self.use_anyup and (not self.anyup_last_only) and (self.anyup is not None) and (hr_image is not None):
            # 用 AnyUp 把 ViT 特征上采样到和 features[2] 同一分辨率
            x = self.anyup(
                hr_image, x,
                output_size=features[2].shape[-2:],
        ...
        if self.use_anyup and (not self.anyup_last_only) and (self.anyup is not None) and (hr_image is not None):
            x = self.anyup(
                hr_image, x,
                output_size=features[3].shape[-2:],
                q_chunk_size=self.anyup_q_chunk_size,
            )
        else:
            x = self.up2(x)
        ...
        if self.use_anyup and (self.anyup is not None) and (hr_image is not None):
            #最终上采样到 features[0] 的分辨率，也是输入图像的分辨率
